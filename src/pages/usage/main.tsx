...
function UsagePage() {
  ...
  return <main className={styles.shell}>
    <header className={styles.header}><a aria-label={t("back")} className={styles.back} href="/"><ArrowLeft aria-hidden="true" size={20} /></a><div><p>{data?.label ?? t("cursor")}</p><h1>{t("usageTitle")}</h1></div><div className={styles.actions}><Tooltip content={t("export")}><button aria-label={t("export")} className={styles.export} disabled={busy} onClick={() => void openExport()} type="button"><FileOutput aria-hidden="true" size={18} /></button></Tooltip><button className={styles.refresh} disabled={busy} onClick={() => void refresh()} type="button"><RefreshCw aria-hidden="true" size={17} />{t("usageRefresh")}</button></div></header>
