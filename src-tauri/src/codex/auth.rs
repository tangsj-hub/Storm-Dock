...
mod tests {
    ...
    fn writes_and_restores_auth_json() {
        let path = env::temp_dir().join(format!("storm-dock-auth-{}.json", uuid::Uuid::new_v4()));
        let _ = fs::remove_file(&path);
        let auth = serde_json::json!({ "OPENAI_API_KEY": "sk-one" });
        write_auth(&path, &auth).unwrap();
        assert_eq!(read_auth(&path).unwrap()["OPENAI_API_KEY"], "sk-one");
